...
fn create_entity_profile_neurons(
    ...
    for entity in &entities {
        ...
        // R31: Accumulate entity facts across sessions instead of overwriting.
        // When multiple sessions mention the same entity (e.g. "Alice"), each session
        // appends its facts to the profile rather than replacing them. This is critical
        // for multi-session retrieval: "What places has Alice visited across all sessions?"
        // needs facts from ALL sessions, not just the last one.
        // Cap at 8000 chars to bound token budget impact.
        const ENTITY_PROFILE_MAX_CHARS: usize = 8_000;
        let merged_excerpts = if profile_path.exists() {
            let existing = std::fs::read_to_string(&profile_path).unwrap_or_default();
            // Extract just the facts section from existing content
            let existing_facts = if let Some(pos) = existing.find("\n## facts\n") {
                existing[pos + 10..].trim_end_matches('\n').to_string()
            } else {
                String::new()
            };
            if existing_facts.is_empty() {
                ...
                // Append new excerpts; trim to cap (keep newest facts = tail)
                let combined = format!("{existing_facts}\n{excerpts}");
                if combined.len() > ENTITY_PROFILE_MAX_CHARS {
                    // Keep only the last ENTITY_PROFILE_MAX_CHARS chars (newest facts)
                    combined[combined.len() - ENTITY_PROFILE_MAX_CHARS..].to_string()
                } else {
                    combined
                }
            }
        ...
        let content = format!(
            ...
            merged_excerpts = merged_excerpts,
        );
