...
mod tests {
    ...
    fn linked_kg_reasoning_index() -> (tempfile::TempDir, NeuronIndex, PathBuf) {
        ...
        assert!(entity.replace_active_fact(
            "occupation",
            "product designer",
    ...
    fn render_answer_output_abstains_without_supported_answer() {
        ...
        let idx = NeuronIndex::default();
        let output =
            render_answer_output(&idx, "What is my job?", &[(path, 7.0)], false, Some(0.3));
        assert!(output.is_none());
    }
    ...
    fn select_answer_prefers_current_kg_temporal_state() {
        ...
        let answer =
            select_answer("What is the reviewer's current status?", &evidence, None).unwrap();
